...
-- -----------------------------------------------------------------------------
-- Embedded HoverFX shim (CLIENT / EXECUTOR)
-- InsertService:LoadAsset is server-only from Roblox perspective; executors inject
-- local scripts → GetObjects empty + Insert blocked. This implements the same
-- preset names HoverFX.attach uses for our attaches (TweenService-only).
-- -----------------------------------------------------------------------------
local function EmbeddedHoverFX_create()
	...
	local attach = function(gui, preset, options)
		...
		if preset == "RippleClick" then
			-- Never index optional APIs (e.g. GetRelativeMousePosition) — some clients error on member read.
			...
			local function spawnRip(input)
				...
				-- Bright ring: reads as a propagating wave edge, not a flat blob.
				local ring = Instance.new("UIStroke")
				ring.Name = "_HFXRippleRing"
				...
				-- Soft trailing echo: second ring, delayed, subtler depth.
				task.defer(function()
					if not gui.Parent then
						return
					end
					local echo = Instance.new("Frame")
					echo.Name = "_HFXRippleEcho"
		...
		-- Mimics HoverFX Wave (dedicated UIScale — never reuse UI hide/responsive UIScale).
		if preset == "Wave" then
			local amt = (options.WaveAmount or 7) * 0.004
			local half = (options.WaveDuration or 0.22) * 0.45
			local waveName = "_HFXWaveScale"
			connect(gui, function()
		...
		if preset == "Sound" then
			...
			if gui:IsA("GuiButton") then
				-- Activated is unreliable on some clients; MouseButton1Click matches real clicks (and touch).
				connect(gui, function()
		...
		-- Fallback: ignore unknown presets (real HoverFX has many more).
	end
...
do
	...
	-- GetObjects works in Studio / some clients; many executors return {} — use InsertService next.
	...
	-- Mirrors Studio server-side toolbox insert — NOT callable from injected LocalScripts.
	local function _hfxTryInsertService()
		...
		local stageName = "__GlassUI_HoverFX_Staging"
	...
	-- Default embedded HoverFX. Set getgenv().__HOVERFX_EXTERNAL = true to try ReplicatedStorage / rbxm / InsertService instead.
...
local function AttachHoverFX(guiObj, preset, options)
	...
	if not ok then
		warn("[HoverFX] AttachHoverFX error: " .. tostring(err))
	end
end
...
-- Shared UI click (HoverFX Sound is only attached on some controls; this covers the rest without double-firing on HoverFX targets.)
...
local function PlayStarlightClickSound()
	...
	if not _starlightClickSoundInst or not _starlightClickSoundInst.Parent then
		...
		_starlightClickSoundInst.Name = "_StarlightUIClick"
...
function Starlight:CreateWindow(WindowSettings)
 ...
 -- coded by justhey the goat
		...
		ElementSettings.ImageSource = ElementSettings.ImageSource or "Material"
		if ElementSettings.CurrentOptions and not ElementSettings.CurrentOption then
			ElementSettings.CurrentOption = ElementSettings.CurrentOptions
		end
 ...
 * (Element.Values.Increment * 10000000)
			...
			function Element:AddInput(NestedSettings, NestedIndex, Parent, ParentIndex)
				...
				task.spawn(function()
					...
					NestedElement.Instance.FocusLost:Connect(function()
						...
						if not ok then
							warn("[Starlight] Input callback error: " .. tostring(err))
						end
					end)
 ...
 --// ENDSUBSECTION
	...
	-- Preview-only 3D hitbox editor dock (visual mannequin + sliders; does not modify gameplay hitboxes).
	function Starlight.Window:CreateHitboxEditorPreview(opts)
		...
		-- Dock visibility follows user intent, not main-window visibility (syncDock no longer forces dock open with mw).
